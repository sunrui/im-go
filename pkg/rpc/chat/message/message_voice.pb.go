...
// 	protoc        v4.25.3
// source: pkg/rpc/chat/message/message_voice.proto
...
type MessageVoice struct {
	...
	//
	//	*MessageVoice_AliasName
	//	*MessageVoice_Hash
	//	*MessageVoice_Url
	Uri isMessageVoice_Uri `protobuf_oneof:"Uri"`
}
...
func (x *MessageVoice) String() string {
...
func (x *MessageVoice) GetAliasName() string {
	if x, ok := x.GetUri().(*MessageVoice_AliasName); ok {
		return x.AliasName
	}
	return ""
}

func (x *MessageVoice) GetHash() string {
	if x, ok := x.GetUri().(*MessageVoice_Hash); ok {
		return x.Hash
	}
	return ""
}

func (x *MessageVoice) GetUrl() string {
	if x, ok := x.GetUri().(*MessageVoice_Url); ok {
		return x.Url
	}
	return ""
}

type isMessageVoice_Uri interface {
	isMessageVoice_Uri()
}

type MessageVoice_AliasName struct {
	AliasName string `protobuf:"bytes,1,opt,name=alias_name,json=aliasName,proto3,oneof"` // 别名
}

type MessageVoice_Hash struct {
	Hash string `protobuf:"bytes,2,opt,name=hash,proto3,oneof"` // 哈希
}

type MessageVoice_Url struct {
	Url string `protobuf:"bytes,3,opt,name=url,proto3,oneof"` // 地址
}

func (*MessageVoice_AliasName) isMessageVoice_Uri() {}

func (*MessageVoice_Hash) isMessageVoice_Uri() {}
...
var file_pkg_rpc_chat_message_message_voice_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_pkg_rpc_chat_message_message_voice_proto_goTypes = []interface{}{
	(*MessageVoice)(nil), // 0: pkg.rpc.chat.message.MessageVoice
}
