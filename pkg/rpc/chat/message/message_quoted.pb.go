...
// 	protoc        v4.25.3
// source: pkg/rpc/chat/message/message_quoted.proto
...
type MessageQuoted struct {
	...
	//
	//	*MessageQuoted_Text
	Type isMessageQuoted_Type `protobuf_oneof:"Type"`
}
...
func (x *MessageQuoted) String() string {
...
func (m *MessageQuoted) GetType() isMessageQuoted_Type {
	if m != nil {
		return m.Type
	}
	return nil
...
func (x *MessageQuoted) GetText() *MessageText {
	if x, ok := x.GetType().(*MessageQuoted_Text); ok {
		return x.Text
	}
	return nil
}

type isMessageQuoted_Type interface {
	isMessageQuoted_Type()
}

type MessageQuoted_Text struct {
	Text *MessageText `protobuf:"bytes,2,opt,name=text,proto3,oneof"` // 文本
}
...
var file_pkg_rpc_chat_message_message_quoted_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_pkg_rpc_chat_message_message_quoted_proto_goTypes = []interface{}{
	(*MessageQuoted)(nil), // 0: pkg.rpc.chat.message.MessageQuoted
	(*MessageText)(nil),   // 1: pkg.rpc.chat.message.MessageText
}
var file_pkg_rpc_chat_message_message_quoted_proto_depIdxs = []int32{
	1, // 0: pkg.rpc.chat.message.MessageQuoted.text:type_name -> pkg.rpc.chat.message.MessageText
